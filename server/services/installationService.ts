...
class InstallationService {
  ...
  async getInstallationStatus(): Promise<{ installed: boolean; canConnect: boolean }> {
  ...
  async validateDatabaseConnection(config: InstallationConfig['database']): Promise<{
    success: boolean;
    error?: string;
  }> {
    try {
  ...
  async runInstallation(config: InstallationConfig): Promise<InstallationProgress> {
  ...
  private async createAdminUser(adminConfig: InstallationConfig['adminUser']): Promise<void> {
    try {
  ...
  private async configureApplication(appConfig: InstallationConfig['application']): Promise<void> {
    try {
      ...
      throw new Error(`Failed to configure application: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async finalizeInstallation(): Promise<void> {
    try {
      ...
      await fs.writeFile(installFile, JSON.stringify(installationData, null, 2));
      ...
      throw new Error(`Failed to finalize installation: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
