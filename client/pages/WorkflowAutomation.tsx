...
export default function WorkflowAutomation() {
  ...
  const loadData = async () => {
    setIsLoading(true);
    try {
      const [workflowData, executionData, bulkData, auditData, platformData, thirdPartyData] = await Promise.all([
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'running':
        return <Badge variant="secondary" className="bg-blue-100 text-blue-800"><Activity className="h-3 w-3 mr-1" />Running</Badge>;
      case 'completed':
  ...
  const mockWorkflows: WorkflowRule[] = [
    ...
    },
    {
      ...
      trigger: { 
        type: 'scheduled',
        schedule: { type: 'weekly', time: '09:00', timezone: 'UTC', daysOfWeek: [1] }
      },
      conditions: [],
      actions: [
        { id: '1', type: 'audit_seo', config: {} },
        { id: '2', type: 'send_email', config: { recipients: ['admin@example.com'], subject: 'Weekly SEO Report' } }
      ],
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      executionCount: 8
    },
    {
      ...
      trigger: { type: 'manual' },
      conditions: [
        { id: '1', field: 'metaDescription', operator: 'is_empty', value: null }
      ],
      actions: [
        { id: '1', type: 'generate_meta', config: { metaType: 'description', useAI: true } }
      ],
      createdAt: new Date().toISOString(),
      updatedAt: new Date().toISOString(),
      executionCount: 42
      ...
      <div className="container mx-auto px-6 py-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{mockWorkflows.filter(w => w.enabled).length}</div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{mockExecutions.filter(e => e.status === 'running').length}</div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{mockPlatforms.filter(p => p.status === 'connected').length}</div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">{mockThirdParty.filter(t => t.status === 'connected').length}</div>
              <p className="text-xs text-muted-foreground">
              ...
              <CardContent>
                <div className="space-y-4">
                  {mockWorkflows.map((workflow) => (
                    <div key={workflow.id} className="border rounded-lg p-4 hover:bg-muted/30">
                      <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center space-x-3">
                            ...
                            <Badge variant="outline">
                              {workflow.trigger.type === 'scheduled' ? (
                                ...
                                <><Zap className="h-3 w-3 mr-1" />Event-based</>
                              ) : (
                                <><Bot className="h-3 w-3 mr-1" />Manual</>
                              )}
                            </Badge>
                          </div>
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem>
                                {workflow.enabled ? <Pause className="h-4 w-4 mr-2" /> : <Play className="h-4 w-4 mr-2" />}
                      ...
                      <div key={execution.id} className="border rounded-lg p-4">
                        ...
                        {execution.status === 'running' && (
                          <div className="space-y-2">
                            <div className="flex justify-between text-sm">
                              <span>Progress</span>
                              <span>{execution.processedItems}/{execution.totalItems} items</span>
                            </div>
                        ...
                        {execution.status === 'completed' && (
                          <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                            <span>Processed {execution.processedItems} items</span>
                            <span>•</span>
                            <span>Completed in {Math.round((new Date(execution.completedAt!).getTime() - new Date(execution.startedAt).getTime()) / 60000)} minutes</span>
                          </div>
                        )}
                      </div>
                    );
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          <div className="w-12 h-12 bg-muted rounded-lg flex items-center justify-center">
                            {platform.type === 'shopify' && <span className="text-green-600 font-bold">S</span>}
                            {platform.type === 'woocommerce' && <span className="text-purple-600 font-bold">W</span>}
                            {platform.type === 'bigcommerce' && <span className="text-blue-600 font-bold">B</span>}
                          </div>
                          <div>
                      ...
                      {platform.syncSettings.autoSync && (
                        <div className="mt-3 pt-3 border-t">
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="text-muted-foreground">Every {platform.syncSettings.syncInterval} minutes</span>
                          </div>
                        </div>
                      )}
                    </div>
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-3">
                          <div className="w-10 h-10 bg-muted rounded-lg flex items-center justify-center">
                            {integration.type === 'google_search_console' && <Search className="h-5 w-5 text-blue-600" />}
                            {integration.type === 'google_analytics' && <BarChart3 className="h-5 w-5 text-orange-600" />}
                            {integration.type === 'semrush' && <TrendingUp className="h-5 w-5 text-green-600" />}
                            {integration.type === 'ahrefs' && <Target className="h-5 w-5 text-red-600" />}
                            {integration.type === 'pagespeed' && <Smartphone className="h-5 w-5 text-purple-600" />}
                          </div>
                          <div>
