...
export function PlatformIntegrations() {
  ...
  const [syncHistory, setSyncHistory] = useState<{ [key: string]: SyncHistory[] }>({});
  ...
  const handleSync = async (integrationId: string, direction: 'import' | 'export' | 'bidirectional') => {
    try {
      setActiveSyncs(prev => ({ ...prev, [integrationId]: { status: 'running', progress: 0 } }));
      ...
      setActiveSyncs(prev => ({ ...prev, [integrationId]: { status: 'completed', progress: 100 } }));
      ...
      setActiveSyncs(prev => ({ ...prev, [integrationId]: { status: 'failed', progress: 0 } }));
    }
  };
  ...
  const getPlatformIcon = (type: string) => {
    switch (type) {
      case 'shopify':
        return <div className="w-8 h-8 bg-green-100 rounded-lg flex items-center justify-center"><span className="text-green-600 font-bold text-sm">S</span></div>;
      case 'woocommerce':
        return <div className="w-8 h-8 bg-purple-100 rounded-lg flex items-center justify-center"><span className="text-purple-600 font-bold text-sm">W</span></div>;
      case 'bigcommerce':
        return <div className="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center"><span className="text-blue-600 font-bold text-sm">B</span></div>;
      case 'magento':
        return <div className="w-8 h-8 bg-orange-100 rounded-lg flex items-center justify-center"><span className="text-orange-600 font-bold text-sm">M</span></div>;
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Connect and sync with your e-commerce platforms</p>
        </div>
        <Dialog open={showConnectDialog} onOpenChange={setShowConnectDialog}>
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              ...
              <DialogDescription>
                Choose a platform and enter your credentials to start syncing products.
              </DialogDescription>
            </DialogHeader>
            <ConnectPlatformForm 
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{integrations.filter(i => i.syncSettings.autoSync).length}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{integrations.filter(i => i.syncSettings.enableWebhooks).length}</div>
            <p className="text-xs text-muted-foreground">
                ...
                <CardContent className="p-6">
                    ...
                    {integration.status === 'connected' && (
                      <>
                        ...
                        <Button 
                          size="sm" 
                          variant="outline"
                          onClick={() => handleSync(integration.id, 'bidirectional')}
                          disabled={!!activeSync}
                        >
                  ...
                  <div className="mt-4 pt-4 border-t">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        <span className="text-muted-foreground">Sync Fields:</span>
                        <p className="font-medium">{integration.syncSettings.syncFields.length} fields</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Conflicts:</span>
                        <p className="font-medium capitalize">{integration.syncSettings.conflictResolution.replace('_', ' ')}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Interval:</span>
                        <p className="font-medium">{integration.syncSettings.syncInterval}min</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Domain:</span>
                        <p className="font-medium">
                          {integration.type === 'shopify' ? integration.credentials.shopifyDomain : 
                           integration.type === 'woocommerce' ? new URL(integration.credentials.wooCommerceUrl || '').hostname :
                           'N/A'}
                        </p>
                      </div>
                    </div>
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-muted-foreground text-center mb-4">
                  Connect your e-commerce platforms to start syncing products and SEO data.
                </p>
                ...
                {[
                  ...
                  <div key={index} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center space-x-4">
                      ...
                      <div>
                        <p className="font-medium">{sync.platform} - {sync.direction}</p>
                        <p className="text-sm text-muted-foreground">
                          {sync.items} items processed • {sync.duration} • {sync.time}
                        </p>
                      </div>
                    </div>
        ...
        <TabsContent value="field-mapping" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">
                Map fields between your local product data and platform-specific fields.
              </p>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Automatically sync all connected platforms at regular intervals
                    </p>
                  </div>
                  <Switch />
                </div>
...
function ConnectPlatformForm({ onConnect, onCancel }: { 
