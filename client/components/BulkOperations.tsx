...
export function BulkOperations({ selectedProducts, onClearSelection }: BulkOperationsProps) {
  ...
  const handleStartOperation = async (type: string, options: any) => {
    try {
      ...
      const { operationId } = await bulkOperationService.startBulkOperation(config);
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'running':
        return <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100"><Activity className="h-3 w-3 mr-1" />Running</Badge>;
      case 'completed':
  ...
  const calculateETA = (operation: BulkOperation) => {
    if (operation.status !== 'running' || operation.processedItems === 0) return null;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage and monitor bulk operations on your products</p>
        </div>
            ...
            <Dialog open={showNewOperationDialog} onOpenChange={setShowNewOperationDialog}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Perform a bulk operation on {selectedProducts.length} selected products.
                  </DialogDescription>
                </DialogHeader>
                <NewOperationForm 
          ...
          <CardContent className="py-4">
            <div className="flex items-center justify-between">
              ...
              <div className="flex space-x-2">
                <Button size="sm" variant="outline" onClick={() => setShowNewOperationDialog(true)}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{operations.filter(op => op.status === 'running').length}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{operations.filter(op => op.status === 'completed').length}</div>
      ...
      <Tabs defaultValue="active" className="w-full">
        ...
        <TabsContent value="active" className="space-y-4">
          {operations.filter(op => op.status === 'running' || op.status === 'paused').map((operation) => {
                  ...
                  <div className="space-y-4">
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => handleCancelOperation(operation.id)}>
                    ...
                    <div className="space-y-2">
                      <div className="flex justify-between text-sm">
                        <span>
                          {currentProgress.processedItems} of {currentProgress.totalItems} items
                        </span>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-medium text-green-600">{currentProgress.successfulItems}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Failed:</span>
                        <p className="font-medium text-red-600">{currentProgress.failedItems}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">Duration:</span>
                        <p className="font-medium">{formatDuration(operation.startedAt)}</p>
                      </div>
                      <div>
                        <span className="text-muted-foreground">ETA:</span>
                        <p className="font-medium">{eta || 'Calculating...'}</p>
                      </div>
                    </div>

                    {/* Recent Errors */}
                    {currentProgress.errors && currentProgress.errors.length > 0 && (
                      <div className="border-t pt-4">
                        <h4 className="text-sm font-medium text-destructive mb-2">Recent Errors</h4>
                        <div className="space-y-1">
                          {currentProgress.errors.slice(-3).map((error, index) => (
                            <div key={index} className="text-xs bg-destructive/10 p-2 rounded">
                              <span className="font-medium">Product {error.productId}:</span> {error.error}
                            </div>
                          ))}
                        </div>
                      </div>
          ...
          {operations.filter(op => op.status === 'running' || op.status === 'paused').length === 0 && (
            <Card>
        ...
        <TabsContent value="history" className="space-y-4">
          {operations.filter(op => op.status === 'completed' || op.status === 'failed' || op.status === 'cancelled').map((operation) => (
            <Card key={operation.id}>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
        ...
        <TabsContent value="queue" className="space-y-4">
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <h3 className="text-lg font-semibold mb-2">Operation queue is empty</h3>
        ...
        <TabsContent value="templates" className="space-y-4">
          <Card>
            <CardContent className="flex flex-col items-center justify-center py-12">
              ...
              <h3 className="text-lg font-semibold mb-2">No operation templates</h3>
              <p className="text-muted-foreground text-center mb-4">
...
function NewOperationForm({ 
  ...
  const operationTypes = [
    { value: 'edit', label: 'Bulk Edit', description: 'Update product fields in bulk' },
    { value: 'export', label: 'Export', description: 'Export products to CSV/Excel' },
    { value: 'optimize', label: 'SEO Optimize', description: 'Optimize SEO for selected products' },
    { value: 'sync', label: 'Platform Sync', description: 'Sync with external platforms' },
    { value: 'audit', label: 'SEO Audit', description: 'Run comprehensive SEO audit' },
    { value: 'delete', label: 'Delete', description: 'Delete selected products' }
  ];
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center space-x-2">
              <Switch 
                ...
                onCheckedChange={(checked) => setOptions({...options, includeSEO: checked})}
              />
            ...
            <div className="space-y-2 mt-2">
              ...
              <div className="flex items-center space-x-2">
                <Switch 
                  ...
                  onCheckedChange={(checked) => setOptions({...options, generateSchema: checked})}
                />
              ...
              <div className="flex items-center space-x-2">
                <Switch 
                  ...
                  onCheckedChange={(checked) => setOptions({...options, preserveExisting: checked})}
                />
      ...
      <div className="bg-muted/30 p-4 rounded-lg">
        <p className="text-sm text-muted-foreground">
          This operation will affect <strong>{selectedProducts.length}</strong> selected products.
        </p>
      </div>
